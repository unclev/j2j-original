...
class AdHoc:
    ...
    def getSid(self):
        ret = time.strftime("%Y%m%dT%H%M%S", time.localtime(time.time())) + \
              "-" + str(self.sid)
        self.sid += 1
        return ret
    ...
    def getCommandsList(self, query):
        for commandNode in self.commands.keys():
            utils.addDiscoItem(query, self.component.cJid,
                               self.commands[commandNode][0], commandNode)
    ...
    def getReplica(self, iq, fro, ID):
        if not fro.full() in self.component.clients.keys():
            ...
            self.component.send(iq)
            return
        command = utils.createCommand(iq, "replicate_vCard", "executing",
                                      self.getSid())
        form = utils.createForm(command, "form")
        utils.addTitle(form, "vCard replication")
        utils.addLabel(form, "\
Are you sure want to replicate your host's vCard to your guest's account?")
    ...
    def setReplica(self, el, iq, sid, fro, ID):
        ...
        commited = xpath.XPathQuery(xPathStr + \
                        "/field[@var='commit_cb']/value").queryForString(el)
        ...
        el.attributes["from"] = self.component.cJid
    ...
    def getStat(self, iq, fro, ID):
        ...
        utils.addLabel(form, "Online Users: " + \
                       str(len(self.component.clients.keys())))
        ...
        upInMinutes = int((upInSecs - upInDays * 3600 * 24 - upInHours * 3600)\
                          / 60)
        upInSecs = int(upInSecs - upInDays * 3600 * 24 - upInHours * 3600 - \
                       upInMinutes * 60)
        utils.addLabel(form, "Uptime: %d days %d hours %d minutes %d seconds"\
    ...
    def getOpts(self, iq, fro, ID):
        ...
        gtalk = False
        if fro.full() in self.component.clients:
        ...
        utils.addCheckBox(form, "autoReplyEnabled",
                          "Enable Auto Reply for ALL guest contacts", opts[4])
        utils.addCheckBox(form, "autoReplyButForward",
                          "Always forward messages to me", opts[2])
        utils.addMemo(form, "replyText", "Text for Auto Reply (1000 chars max)",
                      opts[0])
        self.component.send(iq)
    ...
    def setOpts(self, el, iq, sid, fro, ID):
        ...
        xPathStr = '/command/x[@xmlns="jabber:x:data"]'
        onlyRoster = xpath.XPathQuery(xPathStr + \
                         "/field[@var='onlyRoster']/value").queryForString(el)
        if onlyRoster:
            opts[3] = utils.strToBool(onlyRoster)
        autoReplyEnabled = xpath.XPathQuery(xPathStr + \
                   "/field[@var='autoReplyEnabled']/value").queryForString(el)
        if autoReplyEnabled:
            opts[4] = utils.strToBool(autoReplyEnabled)
        autoReplyButForward = xpath.XPathQuery(xPathStr + \
                "/field[@var='autoReplyButForward']/value").queryForString(el)
        if autoReplyButForward:
            opts[2] = utils.strToBool(autoReplyButForward)
        lightNotify = xpath.XPathQuery(xPathStr + \
